...
#define OPTION(t, p) \
  { t, offsetof(struct __myfs_options_struct_t, p), 1 }

static const struct fuse_opt __myfs_option_spec[] = {
    OPTION("--backupfile=%s", filename), OPTION("--size=%s", size),
...
static int __myfs_setup_environment(struct __myfs_environment_struct_t *env,
                                    struct __myfs_options_struct_t *opts) {
...
int __myfs_getattr_implem(void *, size_t, int *, uid_t, gid_t, const char *,
                          struct stat *);
...
int __myfs_read_implem(void *, size_t, int *, const char *, char *, size_t,
                       off_t);
...
static int __myfs_getattr(const char *path, struct stat *st) {
  ...
  res = __myfs_getattr_implem(env->memory, env->size, &__myfs_errno, env->uid,
                              env->gid, path, st);
...
static int __myfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
  ...
  __myfs_errno = ENOENT;
  pthread_mutex_lock(&(env->env_lock));
  res = __myfs_readdir_implem(env->memory, env->size, &__myfs_errno, path,
                              &names);
  pthread_mutex_unlock(&(env->env_lock));
  if (res >= 0) {
    if (res == 0) {
...
static int __myfs_truncate(const char *path, off_t size) {
  ...
  pthread_mutex_lock(&(env->env_lock));
  res =
      __myfs_truncate_implem(env->memory, env->size, &__myfs_errno, path, size);
  pthread_mutex_unlock(&(env->env_lock));
  if (res >= 0) return res;
  return -__myfs_errno;
}
...
static int __myfs_read(const char *path, char *buf, size_t size, off_t offset,
  ...
  __myfs_errno = ENOENT;
  pthread_mutex_lock(&(env->env_lock));
  res = __myfs_read_implem(env->memory, env->size, &__myfs_errno, path, buf,
                           size, offset);
  pthread_mutex_unlock(&(env->env_lock));
  if (res >= 0) return res;
  return -__myfs_errno;
...
static int __myfs_write(const char *path, const char *buf, size_t size,
                        off_t offset, struct fuse_file_info *fi) {
  struct fuse_context *context;
  struct __myfs_environment_struct_t *env;
  int __myfs_errno, res;
  ...
  __myfs_errno = ENOENT;
  pthread_mutex_lock(&(env->env_lock));
  res = __myfs_write_implem(env->memory, env->size, &__myfs_errno, path, buf,
                            size, offset);
  pthread_mutex_unlock(&(env->env_lock));
  if (res >= 0) return res;
  return -__myfs_errno;
...
static int __myfs_fsync(const char *path, int datasync,
                        struct fuse_file_info *fi) {
  struct fuse_context *context;
  struct __myfs_environment_struct_t *env;
  int __myfs_errno, res;
...
static struct fuse_operations __myfs_operations = {.getattr = __myfs_getattr,
...
int main(int argc, char *argv[]) {
  ...
  */
  if (!__myfs_options.show_help) {
    ...
    if (!__myfs_setup_environment(env_ptr, &__myfs_options)) return 1;
  } else {
